...
// Supported formats are JSONT, XML, and CBOR.
const (
	JSONT Format = 1 + iota
	XML
)
...
var (
	...
	// ErrUnsupportedFormat indicates the wrong message format (format other than JSONT, XML or CBOR).
...
func (n transformer) Transform(msg mainflux.Message) (interface{}, error) {
	...
	raw, err := Decode(msg.Payload, JSONT)
	if err != nil {
		return nil, err
	}
